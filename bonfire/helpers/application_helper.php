...
if (! function_exists('get_words_until')) {
/**
...
if (! function_exists('calculateTimeTaken')) {
  /**
   * @param $time
   * @return array
   */
  function calculateTimeTaken($time)
  {
      ...
      $formattedTime = gmdate("H:i:s", $time); //formatted as minutes:seconds      
      $timeportions = explode(':', $formattedTime);

      $ret['hours'] = $timeportions[0] == '00' ? '' : ltrim($timeportions[0], '0') . ' hours ';
      $ret['mins'] = $timeportions[1] == '00' ? '' : ltrim($timeportions[1], '0') . ' mins ';
      $ret['secs'] = $timeportions[2] . ' secs';
